...
public class Utils {
     ...
     * about the Movies we are looking for
     */
        ...
        try {
            ...
            if (!baseJsonResponse.isNull("results")) {
                ...
                for (int i = 0; i < featureArray.length(); i++) {
                    // Extract out the first item (which is a movie)
                    JSONObject firstFeature = featureArray.getJSONObject(i);
                    String title = firstFeature.getString("title");
                    String releaseDate = firstFeature.getString("release_date");
                    String thumbnail = firstFeature.getString("poster_path");
                    double voteAverage = firstFeature.getDouble("vote_average");
                    String plot = firstFeature.getString("overview");


                    // Create a new {@link Movie} object
                    Movie movie = new Movie(title, releaseDate, thumbnail, voteAverage, plot);
                    movies.add(movie);
                }
